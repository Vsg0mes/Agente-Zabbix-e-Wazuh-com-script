...
# ================================
# VERIFICAR SE AGENTE ZABBIX JÁ ESTÁ INSTALADO
# ================================
echo "Verificando se o agente Zabbix já está instalado..."

# Verificar se algum pacote do agente Zabbix está instalado
AGENT_INSTALLED=""
INSTALLED_AGENT_TYPE=""

if dpkg -l | grep -q "^ii.*zabbix-agent2"; then
    AGENT_INSTALLED="true"
    INSTALLED_AGENT_TYPE="zabbix-agent2"
    echo "✓ Zabbix Agent2 encontrado instalado"
elif dpkg -l | grep -q "^ii.*zabbix-agent"; then
    AGENT_INSTALLED="true"
    INSTALLED_AGENT_TYPE="zabbix-agent"
    echo "✓ Zabbix Agent (legacy) encontrado instalado"
fi
...
if systemctl list-unit-files | grep -q "zabbix-agent2.service"; then
    AGENT_INSTALLED="true"
    INSTALLED_AGENT_TYPE="zabbix-agent2"
    echo "✓ Serviço zabbix-agent2 encontrado"
elif systemctl list-unit-files | grep -q "zabbix-agent.service"; then
    AGENT_INSTALLED="true"
    INSTALLED_AGENT_TYPE="zabbix-agent"
    echo "✓ Serviço zabbix-agent encontrado"
fi

# Se o agente está instalado, perguntar o que fazer

if [[ "$AGENT_INSTALLED" == "true" ]]; then
    echo
    echo "=== AGENTE ZABBIX JÁ ESTÁ INSTALADO ==="
    echo "Tipo de agente instalado: $INSTALLED_AGENT_TYPE"
    echo
    echo "O que você gostaria de fazer?"
    echo "1 - Reinstalar o agente (remove instalação atual e faz nova instalação)"
    echo "2 - Pular para configuração (manter instalação atual e configurar apenas)"
    echo "3 - Sair"
    echo
    read -p "Escolha uma opção (1-3): " REINSTALL_CHOICE
    
    case "$REINSTALL_CHOICE" in
        1)
            echo "Opção selecionada: Reinstalar agente"
            ...
            # Remover pacotes
            sudo apt remove --purge -y zabbix-agent2 zabbix-agent 2>/dev/null || true
            
            # Remover diretórios de configuração (fazer backup primeiro)
            if [[ -d "/etc/zabbix" ]]; then
                sudo cp -r /etc/zabbix "/etc/zabbix.backup.$(date +%Y%m%d_%H%M%S)"
                sudo rm -rf /etc/zabbix
            fi
            
            echo "✓ Instalação anterior removida com sucesso"
            echo "Continuando com a nova instalação..."
            ;;
        2)
            echo "Opção selecionada: Pular para configuração"
            echo "Detectando tipo de agente já instalado..."
            
            # Detectar automaticamente o tipo de agente
            if dpkg -l | grep -q "^ii.*zabbix-agent2"; then
                ...
                echo "ERRO: Não foi possível detectar o tipo de agente instalado"
                exit 1
            fi
            
            echo "Agente detectado: $AGENT_PACKAGE"
            echo "Arquivo de configuração: $CONFIG_FILE"
            echo "Serviço: $AGENT_SERVICE"
            echo "Binário: $AGENT_BINARY"
            
            # Ir direto para a configuração
            echo
            echo "Pulando para configuração do agente existente..."
...
echo "Nenhuma instalação anterior encontrada. Continuando com a instalação..."
